...
# Permit2 message
amount_in = 5 * 10**6  # 5 USDC (6 decimals)
...
# Swap parameters
min_amount_out = 1 * 10**15  # 0.001 ETH (adjusted for 5 USDC, ~$5, with 0.3% fee and slippage)
fee = 3000  # 0.3% fee tier

# Manual encoding
permit_command = codec.encode.permit2_permit(permit_data, signed_message)
transfer_command = (
    b'\x12' + w3.to_bytes(hexstr=usdc_address) + amount_in.to_bytes(32, byteorder='big') + w3.to_bytes(hexstr=ur_address)
)
swap_command = (
    b'\x00' +                                    # V3_SWAP_EXACT_IN
    w3.to_bytes(hexstr=ur_address) +             # Recipient (WETH to router)
    amount_in.to_bytes(32, byteorder='big') +    # amountIn
    min_amount_out.to_bytes(32, byteorder='big') +  # amountOutMin
    w3.to_bytes(hexstr=usdc_address) +           # Path: tokenIn
    fee.to_bytes(3, byteorder='big') +           # Fee
    w3.to_bytes(hexstr=weth_address) +           # Path: tokenOut
    b'\x00'                                      # payerIsSender = false
)
unwrap_command = (
...
trx_params = {
    ...
    "to": ur_address,
    "gas": 300_000,
    "maxPriorityFeePerGas": w3.eth.max_priority_fee * 2,  # Boost priority
    "maxFeePerGas": w3.eth.gas_price * 3,                 # Boost base fee
    "type": '0x2',
    "chainId": chain_id,
    "value": 0,
    "nonce": nonce,
...
try:
    receipt = w3.eth.wait_for_transaction_receipt(trx_hash, timeout=120)  # 2 min timeout
    print(f"Status: {'Success' if receipt['status'] == 1 else 'Failed'}, Gas Used: {receipt['gasUsed']}")
except Exception as e:
    print(f"Transaction timed out or failed: {e}")
    print("Check status on https://basescan.org/tx/" + w3.to_hex(trx_hash))
